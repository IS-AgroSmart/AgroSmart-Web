<template lang="html">
  <section class="user_requests">
    <p><strong>Solicitudes</strong></p>
      <input v-model="opcionFilter" placeholder="Buscar usuarios por nombre...">
      <b-dropdown-text v-if="availableUsers.length === 0">
      ¡Ningún usuario tiene ese nombre o ninguna solicitud registrada! 
      </b-dropdown-text>
      <b-dropdown-divider></b-dropdown-divider>

    <div class="row">
      <b-card  v-for="user in availableUsers" :key="user.pk" class="card">
        <img class="image" v-bind:src="getUserImage()" img-alt="Image"
        tag="article" >
        <b-card-title><strong>{{ user.username }}</strong></b-card-title>
          <b-card-text>Descripcion:<br><br>Correo: <i>{{user.email}}</i><br>Estado:<i> El usuario aun<br> <strong>no ha sido aceptado</strong></i><br><br>

            <b-dropdown text="Acciones" ref="dropdown">
             <b-dropdown-divider></b-dropdown-divider>
            <b-dropdown-item-button v-for="accion in acciones" :key="accion.pk" @click='accionRequest(user,accion)'>
              {{ accion }}
            </b-dropdown-item-button>
            </b-dropdown>
          </b-card-text>
      </b-card>
    </div>
    <b-alert variant="success" show v-if="alert">Accion Realizada con exito</b-alert>
  </section>
</template>

<script lang="js">
import axios from "axios";

  export default  {
    name: 'user-requests',
    
    data:function () {
      return{
        demoUser_img:"data:image/png;base64,",
        users: [],
        acciones:['ACEPTAR','ELIMINAR','BLOQUEAR'],
        opcionFilter:'',
        alert:false,
      }
    },
    

    methods: {
      getUserImage(){
        return this.demoUser_img
      },
      accionRequest(user, accion) {
        if(accion!="ACEPTAR"){  
          let acciong='';
          if(accion=="ELIMINAR"){
            acciong='eliminada'
          }else{
            acciong="bloqueada"
          }
            this.$bvModal.msgBoxConfirm('Esta solicitud perteneciente a '+user.username+" sera " + acciong, {
                    title: '¿Realmente desea '+accion+' la solictud?',
                    okVariant: 'danger',
                    okTitle: 'Sí',
                    cancelTitle: 'No',
                    // hideHeaderClose: false
                })
                .then(value => {
                    if (value)
                        axios.patch("api/users/" + user.pk+"/", {
                          type: "DELETED",
                        },{
                          headers:{ "Authorization": "Token " + this.storage.token },  
                        }).then(() => this.alert=true,
                        this.$router.go(),)
                        .catch(() => {
                            this.$bvToast.toast('Error al ' +accion+' la solicitud, intente mas tarde', {
                                title: "Error",
                                autoHideDelay: 3000,
                                variant: "danger",
                            });
                        });
                });
            }else{
              axios.patch("api/users/" + user.pk+"/", {
                          type: "ACTIVE",
                        },{
                          headers:{ "Authorization": "Token " + this.storage.token },  
                        }).then(() => this.alert=true,
                        this.$router.go(),/*this.created()*/)
                        .catch(() => {
                            this.$bvToast.toast('Error al aceptar la solicitud, intente mas tarde', {
                                title: "Error",
                                autoHideDelay: 3000,
                                variant: "danger",
                            });
                        });
            }
        },
    },
    computed: {
      availableUsers() {
        if(this.opcionFilter!=''){
          return this.users.filter(user=>!user.is_staff && user.type=="DEMO_USER" && user.username==this.opcionFilter)//inludes()
        }else{
        //Mostrando solo los usuarios que no son administradores
        return this.users.filter(user=>!user.is_staff && user.type=="DEMO_USER");
        }
      },

    }, 
    created() {
      if (!this.$isLoggedIn()) {
            this.$router.push("/login");
        }
        axios
            .get('api/users', {
                headers: { "Authorization": "Token " + this.storage.token }
            })
            .then(response => {
              this.users = response.data})
            .catch(error => this.error = error);
    },
}

</script>

<style scoped lang="css">
.button {
  padding: 5px;
  margin: 5px;
}

.user_requests {
  width: 100%;
  height: 100%;
}

.card {
  padding: 5px;
  margin: 30px;
  width: auto;
  height: auto;
}
.row {
  padding: 1px;
  margin: 1px;
}
.image {
  width: 200px;
  height: 180px;
  padding: 5px;
}
</style>
